...
// INCLUDE YOUR CODE HERE
    ...
    if(exp(-beta*(energy_new-energy_old))>1)
      prob_acc=1;
    else
      prob_acc=exp(-beta*(energy_new-energy_old));

    if(rnd.Rannyu()<=prob_acc){
      s[o]=-s[o];
      accepted++;
    }
    attempted++;
...
// INCLUDE YOUR CODE HERE
    energy_old=Boltzmann(s[o], o);
    energy_new=Boltzmann(-s[o], o);
    if(rnd.Rannyu()<1/(1+exp(-beta*(energy_old-energy_new)))){
      s[o] = -s[o];
      accepted++;
    }
    attempted++;
...
// INCLUDE YOUR CODE HERE
  ...
  walker[iu] = u;
  walker[ix]=walker[ix]*walker[ix]*beta;
  walker[im]=m;
  walker[ic]=u*u;
// INCLUDE YOUR CODE HERE
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    stima_x = blk_av[ix]/blk_norm/(double)nspin; //Energy
    glob_av[ix]  += stima_x;
