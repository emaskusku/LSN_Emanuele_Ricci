...
void Generation::merge(int low,int mid,int high){
  ...
  //high: indice piu` grande dell'array da fondere
      ...
      int h,i,j,k;
      SimulAnn b[m_dim]; //Abbiamo bisogno di un vettore di appoggio dove copiare i dati
...
double SimulAnn::prob(){
  return exp(-GetLen()/T);//occhio qui alla T: è una varibile globale e non va bene,
}                         // ripristina il m_T se vuoi usare questa funzione

bool SimulAnn::Metropolis(double p){
  double metro_p=p/prob();
...
void SimulAnn::CopiaPath1(Path pat){
  for(int i=0; i<m_N; i++){
    m_path[i]=pat.GetCity(i);
  }
}
