...
int main (int argc, char *argv[]){
  ...
  int dd=atoi(argv[1]);
  double mpiappox[dd][N];
  double mpiappoy[dd][N];
  /*
  double ** mpiappox ;
  double ** mpiappoy ;
  mpiappox= new double *[dd];
  mpiappoy= new double *[dd];
  for(int i=0; i<dd; i++){
    mpiappox[i]=new double [N];
    mpiappoy[i]=new double [N];
  }
  */
  cout << dd<<endl;
  //cout << argv[2]<<endl;
...
/*
     for(int i=0; i<dd; i++){
        for(int k=0; k<N; k++){
          if(rank==i){
           mpiappox[rank][k]=mygen[j+1].m_gen[0].m_x[k];
           mpiappoy[rank][k]=mygen[j+1].m_gen[0].m_y[k];
           //cout<<mpiappox[rank][k]<<endl;
         }
         else {
            mpiappox[rank][k]=0;
            mpiappoy[rank][k]=0;
     ...
     for(int i=0; i<dd; i++){
       for(int k=0; k<N; k++){
         if(i==rank){
           MPI_Bcast(&mpiappox[rank][k], 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
           //mpiappox[i][k]=a;
         }
         //cout<<mpiappox[rank][k]<<endl;
       }
...
*/
     ...
     for(int i=0; i<dd; i++){
       for(int k=0; k<dd; k++){
         //cout<< "ciao"<<rank<<" "<<i<<" "<<k<<endl;
         if(i!=k){
           if(i==rank){
             //ind=(int)rnd.Rannyu(1,DIM);
             MPI_Isend(&mygen[j+1].m_gen[0].m_x[0],N,MPI_DOUBLE_PRECISION,k,k, MPI_COMM_WORLD,&req);
             MPI_Isend(&mygen[j+1].m_gen[0].m_y[0],N,MPI_DOUBLE_PRECISION,k,k+1, MPI_COMM_WORLD,&req);
           } else if (rank==k){
             ind=(int)rnd.Rannyu(1,DIM);
             MPI_Recv(&mygen[j+1].m_gen[ind].m_x[0],N,MPI_DOUBLE_PRECISION,i,k,MPI_COMM_WORLD, &stat1);
             MPI_Recv(&mygen[j+1].m_gen[ind].m_y[0],N,MPI_DOUBLE_PRECISION,i,k+1,MPI_COMM_WORLD, &stat1);
    ...
    for(int i=0; i<dd; i++){
      for(int k=0; k<dd; k++){
        ...
        if(i!=k){
          if(i==rank){
            //ind=(int)rnd.Rannyu(1,DIM);
            MPI_Isend(&mygen1[j+1].m_gen[0].m_x[0],N,MPI_DOUBLE_PRECISION,k,k, MPI_COMM_WORLD,&req);
            MPI_Isend(&mygen1[j+1].m_gen[0].m_y[0],N,MPI_DOUBLE_PRECISION,k,k+1, MPI_COMM_WORLD,&req);
          } else if (rank==k){
            ind=(int)rnd.Rannyu(1,DIM);
            MPI_Recv(&mygen1[j+1].m_gen[ind].m_x[0],N,MPI_DOUBLE_PRECISION,i,k,MPI_COMM_WORLD, &stat1);
            MPI_Recv(&mygen1[j+1].m_gen[ind].m_y[0],N,MPI_DOUBLE_PRECISION,i,k+1,MPI_COMM_WORLD, &stat1);
